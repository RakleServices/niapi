...
router.delete('/branch/:id', (req, res) => {
    ...
    mysqlConnection.query('delete from branch where bid = ?', [req.params.id], (err, rows, fields) => {
        if (!err)
            res.send({data : true, res:'Branch Deleted Succesfully'});
        else
...
router.delete('/banner1/:id', (req, res) => {
    res.type('json')
    if (
        !req.headers.authorization ||
        !req.headers.authorization.startsWith('Bearer') ||
...
router.post('/banner1',bupload.single('banimage'),  (req, res) => {
...
router.get('/specialoffer', (req, res) => {
    ...
    //     }
    //     console.log(decoded.id);
    mysqlConnection.query('Select * from specialoffer', (err, rows, fields) => {
        res.type('json')
        if (!err)
            if (rows && rows.length) {
                res.send(JSON.stringify({ rowsCount: rows.length, data: rows }));
...
router.delete('/specialoffer/:id', (req, res) => {
    res.type('json')
    // if (
    //     !req.headers.authorization ||
    //     !req.headers.authorization.startsWith('Bearer') ||
    ...
    //     }
    //     console.log(decoded.id);
    mysqlConnection.query('delete from specialoffer where id = ?', [req.params.id], (err, rows, fields) => {
        if (!err)
            res.send({data : true, res:'Banner Deleted Succesfully'});
        else
            res.send({data:false, res:'Something went wrong! Please try again..'});
...
const spstorage = multer.diskStorage({
    destination: './spoffer/',
    filename: (req, file, cb) => {
        return cb(null, `${file.fieldname}_${Date.now()}${path.extname(file.originalname)}`)
    }
...
router.post('/specialoffer',spupload.single('spofferimg'),  (req, res) => {
    const data = { spofferimg: req.file.filename};
    console.log(data);
    mysqlConnection.query('insert into specialoffer set?', [data], (err, rows, fields) => {
        res.type('json')
...
router.get('/product', (req, res) => {
    ...
    //     }
    //     console.log(decoded.id);
    mysqlConnection.query('SELECT product.pid, product.pname, product.pdesc,product.prodimage, product.pprice, product.transdate, product.status,product.counter, branch.bphone, catagory.catname FROM product INNER JOIN catagory ON product.catid = catagory.catid INNER join branch on product.bid = branch.bid where catagory.catid != 33', (err, rows, fields) => {
        res.type('json')
        if (!err)
            if (rows && rows.length) {
...
router.get('/product/:id', (req, res) => {
    ...
    //     }
    //     console.log(decoded.id);
    mysqlConnection.query('SELECT product.pid,product.prodimage, product.pname, product.pdesc, product.pprice, product.transdate,product.counter, product.status, catagory.catname, branch.bphone FROM product INNER JOIN catagory ON product.catid = catagory.catid INNER JOIN branch on product.bid = branch.bid where product.pid=?', [req.params.id], (err, rows, fields) => {
        res.type('json')
        ...
        if (!err)
            if (rows && rows.length) {
                ...
                console.log(counter)
                res.send(JSON.stringify({ rowsCount: rows.length, data: rows }));
                mysqlConnection.query('update product set counter =? where pid =  ?', [counter+1,req.params.id ], (err1, rows1, fields) => {
                    if(!err1){
                        console.log("countter updated succesfully");
                    }
                    else{
                        console.log(err1);
                    }
...
router.delete('/product1/:id', (req, res) => {
    res.type('json')
    // if (
    //     !req.headers.authorization ||
    //     !req.headers.authorization.startsWith('Bearer') ||
...
router.get('/nilamiProducts', (req, res) => {
    ...
    //     console.log(decoded.id);
    var sqlquery = 
    mysqlConnection.query(`select * from product where catid=33 and STR_TO_DATE(nilamidate,'%Y-%m-%dT%H:%i') > CONVERT_TZ(NOW(),'SYSTEM','+05:30')`, (err, rows, fields) => {
        res.type('json')
        if (!err)
            if (rows && rows.length) {
                res.send(JSON.stringify({ rowsCount: rows.length, data: rows }));
